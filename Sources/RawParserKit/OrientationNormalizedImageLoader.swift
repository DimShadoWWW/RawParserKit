...
public enum SupportedFileType: String, CaseIterable {
    ...
    public nonisolated static let renderedImageExtensions: Set<String> = [
    ...
    public nonisolated static func isRenderedImage(_ url: URL) -> Bool {
        renderedImageExtensions.contains(url.pathExtension.lowercased())
    }
}


public enum OrientationNormalizedImageLoader {
    // MARK: - Public API
    ...
    public nonisolated static func loadCGImage(from url: URL) -> CGImage? {
    ...
    public nonisolated static func loadCGImage(from data: Data) -> CGImage? {
        let sourceOptions = [kCGImageSourceShouldCache: false] as CFDictionary
    ...
    // periphery:ignore
    public nonisolated static func loadThumbnail(from url: URL, maxPixelSize: Int) -> CGImage? {
        loadThumbnail(from: url, maxPixelSize: maxPixelSize, createIfAbsent: true)
    }

    public nonisolated static func loadEmbeddedThumbnail(from url: URL, maxPixelSize: Int) -> CGImage? {
        loadThumbnail(from: url, maxPixelSize: maxPixelSize, createIfAbsent: false)
    }

    public nonisolated static func applyingSourceOrientation(to image: CGImage, from url: URL) -> CGImage? {
        let sourceOptions = [kCGImageSourceShouldCache: false] as CFDictionary
        guard let imageSource = CGImageSourceCreateWithURL(url as CFURL, sourceOptions) else {
            return image
        }
    ...
    public nonisolated static func loadSonyEmbeddedPreview(from rawURL: URL) -> CGImage? {
    ...
    public nonisolated static func loadEmbeddedPreview(from data: Data, sourceURL: URL) -> CGImage? {
        let sourceOptions = [kCGImageSourceShouldCache: false] as CFDictionary
        guard let imageSource = CGImageSourceCreateWithData(data as CFData, sourceOptions) else {
            return nil
        }
