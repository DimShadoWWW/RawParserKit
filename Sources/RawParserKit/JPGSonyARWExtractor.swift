...
public enum JPGSonyARWExtractor {
    ...
    private nonisolated static func extractSync(
        ...
        guard let imageSource = CGImageSourceCreateWithURL(arwURL as CFURL, sourceOptions) else {
            RawParserKitLog.process.warning("JPGSonyARWExtractor: Failed to create image source")
            return nil
        }
        ...
        if targetIndex != -1 {
            ...
            if requiresDownsampling {
                RawParserKitLog.process.info("JPGSonyARWExtractor: Native downsampling to \(maxThumbnailSize)px")
                ...
                RawParserKitLog.process.info("JPGSonyARWExtractor: Using original preview size (\(targetWidth)px)")
            ...
            RawParserKitLog.process.warning("JPGSonyARWExtractor: No JPEG found via ImageIO - trying binary fallback")
        }
